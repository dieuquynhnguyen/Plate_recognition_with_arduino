...
def sort_contours(cnts,roi):
    # if not (roi.shape[1] / roi.shape[0] < 2):
    if lp_type == 1:
        ...
        cnts.sort(key=lambda x: get_contour_precedence(x, roi.shape[1]))
    return cnts
...
if (len(LpImg)):
    ...
    # cv2.imshow("Ảnh sau khi chuyển đổi màu sang xám", gray)
    # cv2.waitKey()
    ...
    # cv2.imshow("morphology ", thre_mor)
    # cv2.waitKey()
    ...
    for c in sort_contours(cont,roi):
        ...
        ratio = h / w
        if 1.3 <= ratio <= 6:  # Chon cac contour dam bao ve ratio w/h
         # Chon cac contour cao tu 60% bien so tro len
            if ((0.9>h / roi.shape[0] >= 0.5) or ((roi.shape[1] / roi.shape[0] < 2) and (0.5 > h / roi.shape[0] >= 0.3))):
            # if ((0.9 > h / roi.shape[0] >= 0.5) or (
            #         (lp_type == 2) and (0.5 > h / roi.shape[0] >= 0.35))):
    ...
    print("Biển số nhận diện được",fine_tune(plate_info))
